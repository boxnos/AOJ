...
int main() {
	int h1 {in}, h2 {in}, k1 {in}, k2 {in}, a {in}, b {in}, c {in}, d {in};
	auto calc = [&](int i, int y) {
		return i * a + i / 10 * c + y * b + y / 20 * d;
	};
	int h = calc(h1, h2), k = calc(k1, k2);
	outl(h == k ? "even" : h > k ? "hiroshi" : "kenjiro");
}
