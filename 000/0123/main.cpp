...
#define m(a, b, c) make_tuple(a, b, c)

int main() {
	vector<tuple<string, double, double>> r =
	{m("AAA", 35.5, 71), m("AA", 37.5, 77), m("A", 40, 83), m("B", 43, 89),
	 m("C", 50, 105), m("D", 55, 116), m("E", 70, 148), m("NA", 361, 361)};
	for (double a, b; scanf("%lf %lf", &a, &b) != EOF;)
		for (auto t: r)
			if (a < get<1>(t) && b < get<2>(t)) {
				outl(get<0>(t));
				break;
			}
}
