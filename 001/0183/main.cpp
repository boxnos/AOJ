...
int main() {
	for (;;) {
		...
		if (t[0] == "0")
			break;
		auto check = [&](int ai, int aj, int bi, int bj, int ci, int cj){
			return t[ai][aj] != '+' && t[ai][aj] == t[bi][bj] && t[ai][aj] == t[ci][cj];
		};
		outl([&] () -> string {
			 for (int i: range(3))
				if (t[i] == "bbb" || t[i] == "www")
					return {t[i][0]};
				else if (check(0, i, 1, i, 2, i))
					return {t[0][i]};
			return check(0, 0, 1, 1, 2, 2) || check(2, 0, 1, 1, 0, 2) ? string {t[1][1]} : "NA";
			}());
	}
}
